...
const StepVariants = ({ formData, onChange }) => {
  ...
  // ===== Active attributes (from dummy or API) =====
  const activeAttributes = useMemo(
    () => dummyAttributes.filter((a) => a.status === "active"),
    []
  );
  ...
  useEffect(() => {
    const selectedAttrs = activeAttributes.filter((a) =>
      selectedAttributes.includes(a._id)
    );
    const matrix = selectedAttrs.map((attr) => selectedValues[attr._id] || []);
      ...
      <div className="bg-white border border-border rounded-md p-4">
        ...
        <div className="flex flex-wrap gap-2">
          {activeAttributes.map((attr) => {
            const active = selectedAttributes.includes(attr._id);
            return (
              <button
                key={attr._id}
                type="button"
        ...
        {selectedAttributes.map((attrId) => {
          const attr = dummyAttributes.find((a) => a._id === attrId);
          if (!attr) return null;
          ...
          const values =
            attr.values?.filter(
              (v) =>
                v.status === "active" && v.value.toLowerCase().includes(search)
            ) || [];
