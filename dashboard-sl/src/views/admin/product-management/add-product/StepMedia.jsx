...
const StepMedia = ({ formData, onChange }) => {
  ...
  const colorAttrId = formData?.variantMeta?.selectedAttributes?.find((id) => {
    const attr = dummyAttributes.find((a) => a._id === id);
    return attr?.name?.toLowerCase() === "color";
  });

  const colorAttribute = colorAttrId
    ? dummyAttributes.find((a) => a._id === colorAttrId)
    : null;

  const colorValues = colorAttribute
    ? formData?.variantMeta?.selectedValues?.[colorAttribute._id] || []
    : [];
    ...
    <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 gap-4 mt-3">
      {gallery.map((img) => {
        ...
        return (
          <div
            key={img.id}
            className={`relative group rounded-lg overflow-hidden border transition hover:shadow-md ${
              isFeatured ? "border-main ring-2 ring-main" : "border-gray-200"
            }`}
          >
            <img
              src={img.preview}
